...
def evaluate_test_set(model, cfg):
    ...
    return aps
def predict_single_image(model, img_path, cfg,evaluator):
    detector_name = 'FasterRCNN'
    regressed_rois = None
    cls_probs = None
    regressed_rois, cls_probs = evaluator.process_image(img_path)
    return regressed_rois, cls_probs
def predict_dummy_image(model, cfg,evaluator):
    detector_name = 'FasterRCNN'
    regressed_rois = None
    cls_probs = None
    regressed_rois, cls_probs = evaluator.process_dummy_image()
    return regressed_rois, cls_probs
