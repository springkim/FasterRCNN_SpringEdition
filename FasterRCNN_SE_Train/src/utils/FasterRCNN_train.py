...
from utils.FastRCNN_train import clone_model, clone_conv_layers, create_fast_rcnn_predictor, create_detection_losses
import time
...
def save_epoch_model(loss, image_input, dims_input, cfg,epoch):
    eval_model = create_faster_rcnn_eval_model(loss, image_input, dims_input, cfg)
    model_file_name=cfg['MODEL_PATH'];
    model_file_name=model_file_name.replace(".model","_epoch(" + '{0:08}'.format(epoch+1) + ").model")
    print(model_file_name)
    eval_model.save(model_file_name)
    eval_model.__del__()
    eval_model.__disown__()
    eval_model=None
    del eval_model
    gc.collect()
    #Fxxxin python
def train_model(image_input, roi_input, dims_input, loss, pred_error,
    ...
    bias_learner = momentum_sgd(biases, bias_lr_schedule, mm_schedule, l2_regularization_weight=l2_reg_weight,
                           unit_gain=False, use_mean_gradient=True)
    #if(cfg["BACKUP_MODEL"]!=None):
    #    print(cfg["BACKUP_MODEL"])
    #    trainer = Trainer(load_model(cfg["BACKUP_MODEL"]), (loss, pred_error), [learner, bias_learner])
    #else:
    ...
    for epoch in range(epochs_to_train):       # loop over epochs
        ...
        #save_epoch_model(loss, image_input, dims_input, cfg,epoch)
