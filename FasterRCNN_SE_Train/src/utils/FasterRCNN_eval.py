...
from utils.plot_helpers import load_resize_and_pad,load_dummy_image_resize_and_pad
...
class FasterRCNN_Evaluator:
    ...
    def process_dummy_image(self):
        out_cls_pred, out_rpn_rois, out_bbox_regr, dims = self.process_dummy_image_detailed()
        labels = out_cls_pred.argmax(axis=1)
        regressed_rois = regress_rois(out_rpn_rois, out_bbox_regr, labels, dims)
    ...
    def process_dummy_image_detailed(self):
        _, cntk_img_input, dims = load_dummy_image_resize_and_pad(self._img_shape[2], self._img_shape[1])
